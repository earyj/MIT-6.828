...
static void trap_dispatch(struct Trapframe *tf) {
  ...
  switch (tf->tf_trapno) {
    ...
    case T_SYSCALL:;
      int res = syscall(tf->tf_regs.reg_eax, tf->tf_regs.reg_edx,
                        tf->tf_regs.reg_ecx, tf->tf_regs.reg_ebx,
                        tf->tf_regs.reg_edi, tf->tf_regs.reg_esi);
      if(res < 0) {
        panic("[trap_dispatch] syscall: %e", res);
      }
...
void page_fault_handler(struct Trapframe *tf) {
  ...
  // LAB 3: Your code here.
  if(tf->tf_cs && 0x3 == 0) {
    panic("page fault happened in kernel mode, va %p!\n", fault_va);
  }
