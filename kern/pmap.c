...
static void *boot_alloc(uint32_t n) {
  ...
  if (PADDR((void *)nextfree) > npages * PGSIZE) {
    panic("out of memory!");
  }
...
struct PageInfo *page_alloc(int alloc_flags) {
  ...
  struct PageInfo *res_page = page_free_list;
...
void page_free(struct PageInfo *pp) {
  ...
  if (pp->pp_ref != 0) {
    panic("Try to free a page whose pp_ref is nonzero!\n");
  }

  if (pp->pp_link != NULL) {
    panic("Double free! Try to free a page whose pp_link is not NULL!\n");
  }
...
pte_t *pgdir_walk(pde_t *pgdir, const void *va, int create) {
  ...
  pte_t *pt_beg = (pte_t *)KADDR(PTE_ADDR(*pde_ptr));
  return pt_beg + PTX(va);
}
...
static void boot_map_region(pde_t *pgdir, uintptr_t va, size_t size,
  ...
  while (nPages--) {
    ...
    // 给页表项赋值
    *pte_ptr = PTE_ADDR(pa) | (perm | PTE_P);
    // 从free_list中删除?
...
int page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm) {
  // Fill this function in
  // if va already has a map, remove it; or do nothing
  // increase pp_ref ahead in case va is already maped to pp, so that directly
  // page_remove() might free pp
  pp->pp_ref++;
...
struct PageInfo *page_lookup(pde_t *pgdir, void *va, pte_t **pte_store) {
  ...
  *pte_store = pte_ptr;
  physaddr_t paddr = PTE_ADDR(*pte_ptr);
